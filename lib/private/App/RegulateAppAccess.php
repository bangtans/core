...
use OCP\App\IRegulateAppAccess;
use OCP\ILogger;
use OCP\IUser;

class RegulateAppAccess implements IRegulateAppAccess {
	/** @var ILogger */
	private $logger;

	/** @var array */
	private $appsAvailableForUsers = [];

	/**
	 * RegulateAppAccess constructor.
	 *
	...
	public function setWhitelistedAppsForUser(IUser $user, $apps) {
		if ($user !== null or !\is_array($apps)) {
			$this->appsAvailableForUsers[$user->getUID()] = $apps;
			$this->logger->debug("Set apps accessible for user " . $user->getUID() . ". Here is the list :" . implode(',', $apps));
			return true;
		}
		$this->logger->debug("Cannot set apps available for user. Something went wrong with the user or app list is not provided");
		return false;
	}
	...
	public function getWhitelistedAppsForUser(IUser $user) {
		...
		if (isset($this->appsAvailableForUsers[$user->getUID()])) {
			return $this->appsAvailableForUsers[$user->getUID()];
		}
		return false;
	}
}
